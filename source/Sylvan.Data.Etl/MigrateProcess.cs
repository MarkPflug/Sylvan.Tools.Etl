...
public class MigrateProcess
{
	...
	string GenerateSelect(TableMapping mapping)
	{
		var sw = new StringWriter();
		sw.Write("select ");
		int i = 0;
		var st = mapping.SourceTable;

		foreach (var col in mapping.ColumnMappings)
		{
			var tCol = col.TargetColumn;
			if (tCol == null)
			{
				continue;
			}
			...
			sw.Write('[');
			sw.Write(col.SourceColumn.ColumnName);
			sw.Write(']');
		}
		sw.Write($" from [{st.TableSchema}].[" + st.TableName + "]");
		return sw.ToString();
	}
	...
	static long WriteData(DbDataReader data, DbProvider provider, TableMapping table)
	{
